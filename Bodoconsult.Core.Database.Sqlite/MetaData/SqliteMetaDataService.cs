...
namespace Bodoconsult.Core.Database.Sqlite.MetaData
{
    /// <summary>
    /// Postgres database implementation of a meta data service
    /// </summary>
    public class SqliteMetaDataService : BaseMetaDataService
    {
        ...
        {
            ConnManagerName = "SqliteConnManager";
        }
        ...
        public override void GetMetaData(string connectionString, string entityName, DbCommand cmd, string nameOfPrimaryKeyField = null)
        {
            ...
            IConnManager db = new SqliteConnManager(connectionString);

            table.Sql = cmd.CommandText;

            // Act
            var reader = (SqliteDataReader)db.GetDataReader(cmd);
        ...
        private static string GetFieldMappingFromDb(MetaDataField field, int index)
        {
            ...
            switch (field.DatabaseType.Name.ToLowerInvariant())
            {
                ...
                case "boolean":
                    ...
                    break;
                case "datetime":
                    result = $"var x{index}= (SqliteDateTime)reader.GetProviderSpecificValue({index});\r\n\tif (!x.IsNegativeInfinity) dto.{fieldName}=x;";
                    break;
                case "string":
        ...
        public override string CreateNewEntityCommand()
        {
            ...
            result.AppendLine("");
            result.AppendLine("var cmd = new SqliteCommand(sql);");
            result.AppendLine("");
            result.AppendLine("SqliteParameter p;");
        ...
        public override string CreateDeleteEntityCommand()
        {
            ...
            result.AppendLine("{");
            result.AppendLine($"");
            result.AppendLine($"\tvar cmd = new SqliteCommand(\"DELETE FROM \\\"{Table.Name}\\\" WHERE \\\"{pk.Name}\\\" = @PK\");");
            result.AppendLine($"");
            result.AppendLine($"\tvar p = new SqliteParameter(\"@PK\", {GetFieldTypeFromType(pk.DatabaseType)}) {{Value = {paraName}}};");
